...
public class UrlUtilsDiffblueTest {
  ...
  /**
   * Test {@link UrlUtils#getParent(URL)}.
   * <p>
   * Method under test: {@link UrlUtils#getParent(URL)}
   */
  @Test
  @Category(MaintainedByDiffblue.class)
  @MethodsUnderTest({"URL UrlUtils.getParent(URL)"})
  public void testGetParent() throws MalformedURLException {
    // Arrange and Act
    URL actualParent = UrlUtils.getParent(Paths.get(System.getProperty("java.io.tmpdir"), "test.txt").toUri().toURL());

    // Assert
    String expectedToStringResult = String.join("", "file:", System.getProperty("java.io.tmpdir"));
    assertEquals(expectedToStringResult, actualParent.toString());
  }
